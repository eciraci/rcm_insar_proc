...
usage: rcm_interf_gamma.py [-h] param_yaml
...
def main() -> None:
    ...
    # - Keep intermediate processing outputs
    keep_f = proc_param['global_parameters']['keep_intermediate_outputs']
    ...
    save_dir = make_dir(out_dir, 'Save')
    ...
    intf_filter = proc_param['interf_param']['filter']
    ...
    off_list = [os.path.join(data_dir, x) for x in os.listdir(data_dir)
                if 'offsets' in x in x]
    for o_fl in off_list:
    ...
    print(current_dir)
    print(os.path.isfile(os.path.join(current_dir, processing_parameters_yml)))
